...
class Controller extends BaseController
{
    ...
    public function videos(Request $request){
        $system = System::where('id',1)->get()->first();
        $videos = Video::where('status',1)->paginate(12);
        $menus = Menu::whereNull('parent_id')->orderBy('stt','ASC')->get();
        return view('front-end.videos',compact('system','menus','videos','request'));
    }
    ...
    public function page($url, Request $request){
    	...
    	if($key=='pi'){
            ...
            return response()->view('front-end.404',$data,404);
        }
    }
    ...
    public function sitemap(){
        ...
        ->add(Url::create('dong-ho')->setPriority(0.8))
        ->add(Url::create('franck-muller')->setPriority(0.8))
        ->add(Url::create('patek-philippe')->setPriority(0.8))
        ->add(Url::create('richard-mille')->setPriority(0.8))
        ->add(Url::create('trang-suc')->setPriority(0.8));
        Blog::all()->each(function(Blog $blog) use ($sitemap){
            $sitemap->add(Url::create($blog->url)->setPriority(0.6));
        });
        BlogCate::all()->each(function(BlogCate $blogCate) use ($sitemap){
            $sitemap->add(Url::create($blogCate->url)->setPriority(0.8));
        });
        Product::all()->each(function(Product $product) use ($sitemap){
            $sitemap->add(Url::create($product->url)->setPriority(0.6));
        });
        ProductCate::all()->each(function(ProductCate $proCate) use ($sitemap){
            $sitemap->add(Url::create($proCate->url)->setPriority(0.8));
        });
        Page::all()->each(function(Page $page) use ($sitemap){
            $sitemap->add(Url::create($page->url)->setPriority(0.6));
        });
