...

{
    ...
    public function getProperties(Request $request){
        if(isset($request->str_ids)){
                ...
                for($j=0;$j<$count;$j++){
                    ...
                    $item = ProductCate::where('id',$ids[$j])->with('properties')->get()->first();

                    foreach($item->properties as $propertie){

                        $propertie = $propertie->id;

                        $ids_propertie[$i]= $propertie;
                        $i++;
    ...
    public function postEditProduct(editProductRequest $request, $id){
        ...
        return redirect()->route('products')->with(['flash_level'=>'success','flash_message'=>'Sửa sản phẩm thành công']);
